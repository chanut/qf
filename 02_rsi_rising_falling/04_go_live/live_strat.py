...
class RSIRisingFalling(Strategy):
    ...
    def long_set_entries_exits_array(
        ...
        try:
            self.rsi_is_below = self.indicator_settings_arrays.rsi_is_below[
                ind_set_index
            ]
    ...
    def short_set_entries_exits_array(
        ...
        try:
            self.rsi_is_above = self.indicator_settings_arrays.rsi_is_above[
                ind_set_index
            ]
    ...
    def long_live_evaluate(
        ...
        try:
            ...
            if (prev_prev_rsi > prev_rsi < current_rsi) and (
                current_rsi < self.rsi_is_below
            ):
                logger.info("\n\n")
                logger.info(
                ...
                self.email_sender.email_new_order(
                    message=f"Long Entry time!!! prev_prev_rsi:{prev_prev_rsi} > prev_rsi:{prev_rsi} < current_rsi:{current_rsi} and current_rsi:{current_rsi} < rsi_is_below:{self.rsi_is_below}",
                    entry_filename=None,
